...
for i in range(0, 3):    # Variables
    print(f'INDEX: {index_names_readable[i]}')
    tickers = indicies[i]
    ...
    for ticker in tickers:
        ...
        try:
            ...
            continue
        try:
            df.to_csv(f'screenerTickers{index_names_readable[i]}/{ticker}.csv')
        except Exception as e:
            error_message = str(e)
            print(error_message)
        ...
        excel_file = f'screenerTickers{index_names_readable[i]}/{ticker}.csv'  
        ...
        print (f'Ticker {tickers.index(ticker)}: {ticker}; Returns Multiple against {index_names_readable[i]}: {returns_multiple}\n')
        time.sleep(0.001)
    ...
    for stock in rs_stocks:    
        try:
            df = pd.read_csv(f'screenerTickers{index_names_readable[i]}/{stock}.csv', index_col=0)
            sma = [50, 150, 200]
            for x in sma:
    ...
    print('\n', exportList)
    writer = ExcelWriter(f"ScreenOutput{index_names_readable[i]}.xlsx")
    exportList.to_excel(writer, "Sheet1")
    writer.save()


    # excel_file = f"ScreenOutput{index_names_readable[i]}.xlsx"
